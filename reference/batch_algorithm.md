...
**배정 규칙 (v4.5 — FFD 캠퍼스 단위, 우선순위 사용자 확정):**
...
구현(FFD — First-Fit Decreasing, 캠퍼스 큰 순): 각 캠퍼스를 ① 통째로 들어가는 호차 중
잔여 최소(best-fit)에 배정 → ② 어느 호차에도 통째로 못 들어가면 잔여 큰 호차부터 분할
(1명 조각 방지) → ③ 정원(44) 다 차면 보조석(45) → 그래도 넘치면 미배정.
v4.4 순차 채움(next-fit) 대비 작은 캠퍼스의 불필요한 분할을 제거(빈좌석은 동일 — 파레토 개선).
더미 300 시뮬 기준 같은요일 분할 3→0, 정원턱(396) 6→1(전남대 등 정원 초과 캠퍼스만).
차량순장·고정탑승은 그 호차에 둘 뿐 같은 캠퍼스를 끌지 않음(순장은 master 개별 배정).
