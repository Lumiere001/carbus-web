...
**배정 규칙 (v4.7 — FFD 캠퍼스 단위, 우선순위 사용자 확정):**
...
5. **(후순위) 1호차 빈자리 최대 [v4.7]** — 1호차는 지구 짐을 함께 실어 인원을 최소화.
   위 1~4를 모두 지킨 뒤, 빈자리 분배에서만 1호차를 **가장 나중에** 채운다(오버플로우 전용).
   다른 호차로 충분하면 1호차는 비워지고, 좌석이 부족하면 그제서야 1호차로 넘긴다
   (미배정 0 유지). `engine.ts`의 `FILL_LAST_BUS_NAMES` — 모든 채움 단계 정렬에 후순위 키.
